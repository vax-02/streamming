<template>
  <div class="flex h-screen bg-gray-800 text-gray-200">
    <main class="flex-1 overflow-y-auto p-8 space-y-12">
      <section id="panel" class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <!-- Active Users Card -->
          <div class="bg-gray-900 border border-gray-700/50 p-6 rounded-2xl text-center hover:scale-105 transition-transform duration-300 ease-out shadow-lg hover:shadow-xl">
            ...
            <p class="text-sm font-medium text-gray-400 uppercase tracking-wider">Usuarios activos</p>
          ...
          <div class="bg-gray-900 border border-gray-700/50 p-6 rounded-2xl text-center hover:scale-105 transition-transform duration-300 ease-out shadow-lg hover:shadow-xl">
            ...
            <p class="text-sm font-medium text-gray-400 uppercase tracking-wider">Streams en Vivo</p>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
             ...
             >
               <div class="relative aspect-video bg-gray-700 overflow-hidden">
                  <div class="absolute inset-0 flex items-center justify-center text-gray-500 group-hover:scale-110 transition-transform duration-500">
                    <VideoCameraIcon class="w-12 h-12 opacity-50" />
                  </div>
                  <div class="absolute top-2 left-2 bg-red-600 text-white text-xs font-bold px-2 py-0.5 rounded animate-pulse">
                    EN VIVO
                  </div>
               </div>
               <div class="p-5">
                  <h3 class="font-bold text-lg text-white mb-1 group-hover:text-blue-400 transition-colors">{{ stream.name }}</h3>
        ...
        <section id="grupos" class="lg:col-span-2 space-y-6">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            >
              <div class="w-16 h-16 rounded-2xl overflow-hidden bg-blue-600/10 flex-shrink-0 border border-gray-700/50">
                <img :src="group.photo || getGroupAvatar(group.name)" class="w-full h-full object-cover">
              </div>
              <div class="flex-1 min-w-0">
                <h3 class="font-bold text-lg text-white group-hover:text-blue-400 transition-colors truncate">
                  {{ group.name }}
                </h3>
              ...
              <div class="text-right flex-shrink-0">
                ...
                <span class="text-[10px] text-gray-500 uppercase font-bold tracking-tighter">Miembros</span>
              </div>
            </div>
          </div>
        </section>
    ...
    <Transition name="modal">
      <div v-show="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
        <div class="bg-gray-900 border border-gray-700 w-full max-w-4xl max-h-[90vh] rounded-3xl overflow-hidden flex flex-col shadow-2xl">
          <!-- Modal Header -->
          <header class="p-6 border-b border-gray-800 flex items-center justify-between bg-gray-900/50">
            <div class="flex items-center gap-4">
              <div class="w-14 h-14 rounded-2xl overflow-hidden bg-blue-600/20 flex items-center justify-center text-blue-400 text-2xl flex-shrink-0 border border-blue-500/20">
                <img :src="selectedGroupData?.photo || getGroupAvatar(selectedGroupData?.name)" class="w-full h-full object-cover">
              </div>
              <div class="min-w-0">
                <h2 class="text-2xl font-bold text-white truncate">{{ selectedGroupData?.name }}</h2>
                <p class="text-sm text-gray-400 truncate">{{ selectedGroupData?.description }}</p>
              </div>
            </div>
            <button @click="closeModal" class="p-2 hover:bg-gray-800 rounded-full transition-colors text-gray-400 hover:text-white">
          ...
          <div class="flex-1 overflow-hidden flex flex-col lg:flex-row relative">
            <!-- Loading Overlay -->
            <div v-if="loadingGroupDetails" class="absolute inset-0 z-10 bg-gray-900/60 backdrop-blur-sm flex flex-col items-center justify-center">
            ...
            <div class="flex-1 p-6 overflow-y-auto border-r border-gray-800 space-y-4">
              ...
              <div v-if="!loadingGroupDetails && (!selectedGroupData?.messages || selectedGroupData.messages.length === 0)" class="h-64 flex flex-col items-center justify-center text-gray-500 italic">
              ...
              <div v-for="(group, date) in groupedMessages" :key="date" class="space-y-4">
                <div class="flex items-center gap-4 my-6">
                  ...
                  <span class="text-xs font-bold text-gray-500 uppercase tracking-widest">{{ date }}</span>
                  <div class="flex-1 h-px bg-gray-800"></div>
                </div>
                ...
                >
                  <img :src="msg.userPhoto || 'https://via.placeholder.com/40'" class="w-10 h-10 rounded-full object-cover">
                  <div class="flex-1">
            ...
            <div class="w-full lg:w-72 p-6 overflow-y-auto bg-gray-900/30">
              ...
              <div class="space-y-3">
                ...
                >
                  <img :src="member.photo || 'https://via.placeholder.com/32'" class="w-8 h-8 rounded-full border border-gray-700 object-cover flex-shrink-0">
                  <div class="flex-1 min-w-0">
    ...
    <Transition name="modal">
      <div v-show="showStreamModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
        <div class="bg-gray-900 border border-gray-700 w-full max-w-md rounded-3xl overflow-hidden shadow-2xl p-8 text-center">
          <div class="w-20 h-20 bg-red-500/10 rounded-full flex items-center justify-center mx-auto mb-6">
            <VideoCameraIcon class="w-10 h-10 text-red-500 font-bold" />
          </div>
          
          <h3 class="text-2xl font-bold text-white mb-2">¿Ingresar a la reunión?</h3>
          <p class="text-gray-400 mb-8 leading-relaxed">
            Estás a punto de unirte a <span class="text-white font-semibold">"{{ selectedStream?.name }}"</span>. 
            ¿Deseas continuar?
          </p>
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <button 
              @click="joinStream" 
              class="px-6 py-3 rounded-2xl bg-red-600 text-white font-semibold hover:bg-red-500 hover:shadow-lg hover:shadow-red-600/20 transition-all active:scale-95"
            >
              Unirse ahora
            </button>
          </div>
  ...
  methods: {
    ...
    joinStream() {
      if (this.selectedStream) {
        this.$router.push(`/live-viewer/${this.selectedStream.socketId}`);
      }
