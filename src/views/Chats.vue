  ...
  <div class="flex h-screen bg-gray-800 text-white font-sans overflow-hidden">
    ...
    >
      ...
      <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-2 custom-scrollbar">
        ...
        >
           <div class="relative">
              <img :src="chat.foto" alt="perfil" class="w-10 h-10 rounded-full object-cover border-2 border-gray-600 group-hover:border-gray-500 transition-colors" />
               <span v-if="chat.online" class="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-gray-800 rounded-full"></span>
           </div>
          ...
          <span v-if="chat.nuevos > 0" class="bg-blue-600 text-white text-xs font-bold px-2 py-0.5 rounded-full shadow-sm">
            {{ chat.nuevos }}
          </span>
        </div>
         ...
         <div v-if="chats.length === 0" class="text-center py-10 px-4 text-gray-500">
             ...
             <button @click="addChatModal = true" class="text-blue-400 text-sm hover:underline mt-2">Iniciar uno nuevo</button>
         </div>
      </nav>
    </aside>
    ...
    >
      ...
      <template v-if="chatSeleccionado">
        ...
        <header class="flex items-center justify-between px-6 py-4 bg-gray-800 border-b border-gray-700 shadow-sm z-10">
             ...
             <button
              ...
              <div class="flex items-center space-x-3 cursor-pointer group" @click="mostrarPerfil = !mostrarPerfil">
                  <div class="relative">
                      ...
                      />
                      <span v-if="chatSeleccionado.online" class="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-gray-800 rounded-full"></span>
                  </div>
                <div>
                  <h2 class="text-lg font-bold text-white leading-tight group-hover:text-blue-400 transition-colors">{{ chatSeleccionado.nombre }}</h2>
          ...
          <div class="flex items-center space-x-2">
                <button class="p-2 text-gray-400 hover:text-white hover:bg-gray-700 rounded-lg transition" title="Videollamada">
               ...
               <button @click="mostrarPerfil = !mostrarPerfil" class="p-2 text-gray-400 hover:text-white hover:bg-gray-700 rounded-lg transition" title="Info">
        ...
        <div class="p-4 bg-gray-800 border-t border-gray-700">
           <div class="flex items-center space-x-2 bg-gray-900 p-1.5 rounded-xl border border-gray-600 focus-within:border-blue-500 transition-colors">
              <input
              ...
              <button 
                @click="sendMessage(chatSeleccionado.id)" 
      ...
      <template v-else>
         <div class="flex-1 flex flex-col items-center justify-center p-8 text-center text-gray-500 bg-gray-900">
             ...
             <p class="max-w-md text-gray-400">Selecciona un chat de la lista para comenzar a enviar mensajes o inicia una nueva conversación.</p>
             <button @click="addChatModal = true" class="mt-8 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-semibold shadow-lg transition-transform hover:-translate-y-1">
            ...
            <div class="max-h-60 overflow-y-auto custom-scrollbar space-y-1">
              ...
              >
                <div class="flex items-center space-x-3">
                  ...
                  <div>
                    <p class="text-sm font-bold text-white group-hover:text-blue-400 transition-colors">{{ contacto.nombre }}</p>
    ...
    <transition name="slide">
      <div
        v-if="mostrarPerfil && chatSeleccionado"
        ...
        <div class="flex flex-col items-center mb-8">
             <div class="relative mb-4">
                 <img
                  ...
                  />
                   <span v-if="chatSeleccionado.online" class="absolute bottom-2 right-2 w-5 h-5 bg-green-500 border-4 border-gray-800 rounded-full"></span>
             </div>
    ...
    <div v-if="mostrarPerfil && chatSeleccionado" class="fixed inset-0 bg-black/50 z-20 md:hidden" @click="mostrarPerfil = false"></div>
